...
%	SECTION 6.1
%-------------------------------------------------------------------------------
\section{Il \textit{Setup} della Piattaforma}
...
riferimento per l'\textit{encoder} di HEVC.
%-------------------------------------------------------------------------------
%	SECTION 6.1.1
%-------------------------------------------------------------------------------
...
funzioni più pesanti dell'\textit{encoder} HM. Nello specifico, abbiamo 
utilizzato lo strumento Callgrind attraverso il comando seguente. \\
...
Dopo aver riconosciuto le funzioni con un maggior peso si è passati alla 
classificazione di quest'ultime. Il grafico che segue descrive il peso dei 
principali moduli dell'\textit{encoder} di HM. \\

\begin{tikzpicture}
  \centering
  \piechartthreed[scale=.8,
                  background color = white,
                  mix color = darkgray]
                  {220/green,65/blue,45/red,30/orange}
                  
  \foreach \i in {1,...,4} { \fill (pc \i) circle (.5mm);}
    \draw[darkgray] (pc 1)  -- ++(-2.5,0) coordinate (s1) 
    node[anchor=south east] {Motion Estimation}
    node[anchor=north east] {57.24\%};
    
    \draw[darkgray] (pc 2)  -- ++(-2.2,-1) coordinate (s2) -- (s2 -| s1) 
    node[anchor=south east] {18.03\%}
    node[anchor=north east] {Motion Compensation};
    
    \draw[darkgray] (pc 3)  -- ++(+1.5,-1.3) coordinate (s3) -- (s2 -| s3)
    node[anchor=south west] {14.74\%}
    node[anchor=north west] {Transform \& Quant.};
    
    \draw[darkgray] (pc 4)  -- ++(+1.5,0) coordinate (s4) 
    node[anchor=south west] {Miscellaneous\footnotemark}
    node[anchor=north west] {9.99\%};
    
\end{tikzpicture}

\footnotetext{SAO, In-Loop Filter, CABAC, Intra prediction.}
...
%,come discusso nella sezione -ref-, 
descrive la codifica digitale del colore. 
Oggi, \verb|yuv| è utilizzato per quelle sequenze video i cui colori sono 
rappresentati in YCrCb.
In sostanza un file con estensione \verb|yuv| contiene semplicemente il 
contenuto binario dei frame, uno dopo l'altro, senza altre informazioni. \`E 
compito dell'utilizzatore specificare il formato di subsampling e la 
risoluzione corretti. \\
...
comune personal computer x86, e l'utilizzo di più risorse - principalmente 
memoria, sia volatile sia di massa. \\
...
%       SUBSECTION 6.4.1
%-----------------------------------
\subsection{Opzioni di Compilazione}\label{ref-compiling-optionos}
...
codice. \\
%non si può modificare direttamente il program counter, e lo scopo primario è 
%quello di visualizzare il valore delle variabili, non di cambiarlo. Un 
%generico 
%controllare è perfetto visto che include entrambi i significati ma ha una 
%certa tendenza per il primo.
...
%fare in modo che il 
%compilatore ottimizzi il codice basandosi sulla conoscenza del codice sorgente.
%Non tutte le ottimizzazioni sono controllabili direttamente via \emph{flag}. 
%questa la tolgo perché è un'affermazione a sé stante senza esempi.
...
\end{itemize}
\paragraph{Principali opzioni di compilazione indipendenti dall'hardware}
...
\begin{itemize}
  ...
  fine di diminuire il tempo di esecuzione del codice.\\ %il tempo non si 
  %migliora, portanna.
...
\paragraph{Opzioni salienti di compilazione specifiche per l'hardware ARM}
...
configurazioni.\\
% Se il motivo non è l'approssimazione (posso vederla solo causata da un 
%decremento dei bit per il calcolo, in modo da velocizzarlo, tipo nel calcolo 
%parallelo o nelle moltiplicazioni), fatemelo sapere in tempo.
...
dalle funzioni della libreria standard C++11, abbiamo aggiunto 
...
\begin{minipage}{.565\textwidth}
  \lstinputlisting{Codes/armgccclamp.s}
\end{minipage}
\hfill
...
\textit{testing}, dopo l'implementazione effettiva all'interno di HM la 
performance dell'\textit{encoder} è calata, come mostra la tabella.
...
funzione Assembly \textit{inline}, GCC è costretto ad inserire un prologo ed un 
epilogo in cui salva e carica lo stato del programma, mentre nella serie di 
test ciò non è necessario.\newline
...
Tra le varie ottimizzazioni che implicavano la modifica del codice, abbiamo
dato largo spazio alle SIMD di ARM: le cosiddette NEON.\\
La tecnologia SIMD permette di accelerare le operazioni matematiche svolte su 
più dati consentendo di effettuare i calcoli in parallelo.\\
In particolare, l'architettura NEON introdotta con ARMv7 possiede 32 registri a 
64-bit interpretati 
come vettori di elementi detti \emph{lane}. I registri possono essere 
accoppiati in 
modo da funzionare come un unico vettore da 128 bit, le cui configurazioni 
possibili sono:

\begin{itemize}
  \item 2 \emph{lane} da 64 bit ciascuno.
  \item 4 \emph{lane} da 32 bit ciascuno.
  \item 8 \emph{lane} da 16 bit ciascuno.
  \item 16 \emph{lane} da 8 bit ciascuno.
\end{itemize}

Tutti gli elementi di un vettore devono essere omogenei, nello 
specifico:
...
\verb|-mfloat-abi=softfp|. Una spiegazione più approfondita delle sopracitate 
opzioni è presente nella sezione \ref{ref-compiling-optionos}.
\\ \\ 
Avendo controllato, \emph{log} di 
\verb|-fopt-info-vec-optimized| alla mano, tutte le funzioni precedentemente 
rintracciate come le più pesanti, abbiamo osservato che la quasi totalità 
di quelle relative alle \emph{Sum of Absolute Differences} (SAD) non viene
vettorizzata automaticamente da GCC. Il lavoro con le \emph{intrinsic} è 
iniziato da lì. Segue la spiegazione dei diversi passaggi svolti per la 
vettorizzazione manuale di tali funzioni, a partire dal codice originale di 
\verb|xGetSAD8|. Abbiamo replicato il procedimento in maniera analoga per tutte 
le altre funzioni, e non verrà pertanto ripetuto. Qualora se ne avesse 
l'interesse, si ricorda che l'intero progetto è 
online\footnote{Reperibile all'indirizzo https://github.com/C-piace-il-C/HM} e 
...
Abbiamo osservato come il metodo più veloce per il calcolo della somma di 
tutte le componenti del vettore sia l'utilizzo di \verb|vgetq_lane_s16| su 
ogni lane di \verb|v_iSum|.
Degno di nota il fatto che a partire da ARMv8 sia stata 
inserita l'\emph{intrinsic} \verb|vaddvq_s16(int16x8_t a)| dedicata proprio a 
questa operazione.
\\ \\
...
versione \emph{fast}. Siamo riusciti a vettorizzare in maniera soddisfacente 
questa categoria di funzioni solo parzialmente ma, essendo tra le più onerose, 
il miglioramento prestazionale è stato comunque significativo.
\\ \\
...
d[15] = m[15] - m[11];
\end{lstlisting}
%non serve mettere il /*...*/ qui in quanto hai già detto che è un estratto
%e non si tratta di parti la cui assenza compromentte la comprensione del codice

Dove \verb|Int| e \verb|TCoeff| sono entrambi del tipo \verb|int32_t| 
(rappresentanti un intero \emph{signed} a 32-bit). Segue la versione 
ottimizzata.\\
%In questo caso è stata mostrata solo la porzione di codice che ha subito 
%modifiche per un migliore raffronto.
%Lo hai fatto anche nel caso precedente
\begin{lstlisting}[language=C]
Int k;
Distortion satd = 0;
...
In aggiunta alle precedenti \emph{intrinsic} abbiamo utilizzato le seguenti:
...
In particolare, tra le funzioni più onerose, tali costrutti compaioni 
soprattutto in \verb|filter|, di cui vengono mostrati a seguire due frammenti 
di codice.
\begin{lstlisting}
c[0] = coeff[0];
c[1] = coeff[1];
...
viene infatti effettuato un solo \emph{jump} all'istruzione corretta. Le 
seguenti sono già riordinate in modo tale da velocizzare il più possibile 
l'esecuzione del 
codice.\\
...
\begin{itemize}
	...
	codice, o, addirittura, in posizioni diverse, a patto di non modificare i 
	...
	che la funzione restituisca \verb|void|.
...
\paragraph*{WPP} Questa tecnica consiste nella parallelizzazione della parte di 
...
ma nei dati, perché per raggiungere lo \textit{speed-up} massimale è necessario 
che i frame intra siano disposti secondo una certa logica. Tuttavia, per 
garantire buoni livelli di parallelismo, è sufficiente che il valore di 
...
\emph{multithread}. Il fattore di \textit{speed-up} è di circa $1.9$. Si noti 
comunque che i valori mostrati sono una media in cui, per motivi di tempo, il 
numero di prove non è molto alto, mentre la variabilità è elevata (abbiamo 
registrato un massimo di $20$ secondi di differenza tra due cicli di encoding 
della stessa sequenza con le medesime configurazioni).
